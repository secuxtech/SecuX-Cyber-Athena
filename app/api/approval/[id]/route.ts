/**
 * Transaction Approval Details API - Retrieve approval details by transaction ID
 * Returns specific transaction approval information for authenticated users
 */
...
async function handleGetApprovalById(req: NextRequest): Promise<NextResponse> {
  ...
  const approvals = await db.transactionApproval.findMany({ where: { txSN: id } });
